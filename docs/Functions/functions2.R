...
cleaning = function(Scores){
  ...
  mutate(home_team = case_when(
    ...
    home_team == "Commanders" ~ "Washington Commanders",
  ...
  mutate(away_team = case_when(
    ...
    away_team == "Commanders" ~ "Washington Commanders",
...
cleaning3 = function(Scores){
  Scores %>% 
    mutate(home_team = case_when(
      ...
      home_team == "Washington" ~ "Washington Commanders",
...
#Function to get weekly results for west point math
wp_results_fn = function(x,y){
  x %>% select(Prediction, Winner = winner, votes_for, votes_against) %>% 
    mutate(Correct = ifelse(Prediction =="Tie", "--", ifelse(Prediction == Winner, "Yes", "No"))) %>%
    mutate(`Correct Votes` = ifelse(Prediction == Winner, votes_for, ifelse(Winner == "TIE", 0, votes_against))) %>% 
    mutate(`Correct Percent` = round(as.numeric(`Correct Votes`)/(dim(wp.picks[[length(wp.picks)]])[1]),4)) %>%
    add_column(Game = 1:y) %>% 
    select(Game, Prediction, Winner, Correct, `Correct Votes`, `Correct Percent`)
}  
...
comb_results_fn = function(x,y){
  x %>% select(Prediction, Winner = winner, votes_for, votes_against) %>% 
    mutate(Correct = ifelse(Prediction =="Tie", "--", ifelse(Prediction == Winner, "Yes", "No"))) %>%
    mutate(`Correct Votes` = ifelse(Prediction == Winner, votes_for, ifelse(Winner == "TIE", 0, votes_against))) %>% 
    mutate(`Correct Percent` = round(as.numeric(`Correct Votes`)/(dim(inscom.picks[[length(inscom.picks)]])[1]+dim(wp.picks[[length(wp.picks)]])[1]),4)) %>%
    add_column(Game = 1:y) %>% 
    select(Game, Prediction, Winner, Correct, `Correct Votes`, `Correct Percent`)
}  
